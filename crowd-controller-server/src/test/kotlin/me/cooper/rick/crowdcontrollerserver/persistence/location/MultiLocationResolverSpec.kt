...
import me.cooper.rick.crowdcontrollerserver.persistence.location.util.LatLonDeg
...
internal object MultiLocationResolverSpec : Spek({
    ...
    given("a resolver and a set of users and a set of expected results") {
        val multiLocationResolver = MultiLocationResolver(MIN_USER_PERCENTAGE, MAX_DISTANCE_METRES)
        val users = mapToUser(mapOf(
                "1" to LatLonDeg(53.485804, -2.273955),
                "2" to LatLonDeg(53.485363, -2.274443),
                "3" to LatLonDeg(53.485823, -2.274722),
                "4" to LatLonDeg(53.486206, -2.273687),
                "5" to LatLonDeg(53.502470, -2.305452), // outside
                "6" to LatLonDeg(53.481891, -2.316782)  // outside
        ))
    ...
    given("a group with users in and a list expected averages") {
        ...
        // @formatter:off
        val users = mapToUser(mapOf(
                "1" to LatLonDeg(8678.123123,   -14232.151254),
                "2" to LatLonDeg(-1245.13123,   124315.05683452),
                "3" to LatLonDeg(-21.0,         -3636.2323),
                "4" to LatLonDeg(4.2315,        125.0),
                "5" to LatLonDeg(2352356.2151,  -56734.2351),
                "6" to LatLonDeg(2352.13525,    546238.235235)
        ))

        val group = Group(null, users.first(), users)
        val usersToExpectedAverageLocation = mapOf(
                listOf("1", "2", "3", "4", "5", "6") to LatLonDeg(393687.4289572,	99345.9455693),
                listOf("1", "2", "3", "4") to           LatLonDeg(1854.0558483,     26642.9183201),
                listOf("2", "3") to                     LatLonDeg(-633.0656150,     60339.4122673),
                listOf("3", "4", "5", "6") to           LatLonDeg(588672.8954625,	121498.1919588),
                listOf("4", "5") to                     LatLonDeg(1176180.2233000,  -28304.6175500),
                listOf("1", "3", "5", "6") to           LatLonDeg(590841.3683683,	117908.9041453),
                listOf("2", "4", "6") to                LatLonDeg(370.4118400,	    223559.4306898),
                listOf("1", "3", "6") to                LatLonDeg(3669.7527910,     176123.2838937),
                listOf("3", "6") to                     LatLonDeg(1165.5676250,     271301.0014675)
        )
        // @formatter:on
        usersToExpectedAverageLocation.forEach { userNames, expectedAverage ->
            on("building an average location dto for $userNames") {
                ...
                it("'s latitude: $lat should be approximately equal to ${expectedAverage.first}") {
                    assertEquals(expectedAverage.first, lat!!, LAT_LONG_ERROR_MARGIN)
                }
                it("'s latitude: $long should be approximately equal to ${expectedAverage.second}") {
                    assertEquals(expectedAverage.second, long!!, LAT_LONG_ERROR_MARGIN)
                }
            }
        }
    }
...
private fun mapToUser(userNameToLocationMap: Map<String, LatLonDeg>): MutableSet<User> {
